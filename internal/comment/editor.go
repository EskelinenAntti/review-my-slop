...
func CreateDraft(state, body string, anchor Anchor) (string, error) {
	...
	path := file.Name()
	if _, err := file.WriteString(Draft(body, anchor)); err != nil {
		_ = file.Close()
		_ = os.Remove(path)
...
func ReadDraft(path string, anchor Anchor, editorErr error) (string, error) {
	...
	return StripSuggestion(string(body), anchor.QuotedLines), nil
}
...
func StripSuggestion(body string, quoted []string) string {
	if len(quoted) == 0 {
		return body
	}
