...
func TestSplitPairsUnequalChangeBlocksAndKeepsHunksSeparate(t *testing.T) {
	p := git.Patch{Files: []git.File{{DisplayPath: "file", Hunks: []git.Hunk{
		{Header: "one", Lines: []git.Line{{Kind: git.Deletion, Text: "d1", OldNumber: 1}, {Kind: git.Deletion, Text: "d2", OldNumber: 2}, {Kind: git.Addition, Text: "a1", NewNumber: 1}, {Kind: git.Addition, Text: "a2", NewNumber: 2}, {Kind: git.Addition, Text: "a3", NewNumber: 3}, {Kind: git.Context, Text: "c", OldNumber: 3, NewNumber: 4}}},
		{Header: "two", Lines: []git.Line{{Kind: git.Addition, Text: "separate", NewNumber: 5}}},
	}}}}
...
func TestSplitSelectionOnlyIncludesActivePane(t *testing.T) {
	...
	if !ok || len(v.Lines(left)) != 1 || v.Lines(left)[0].Kind != git.Deletion {
		t.Fatalf("left lines = %#v", v.Lines(left))
	}
	right := v.BeginSelection(added)
	if len(v.Lines(right)) != 1 || v.Lines(right)[0].Kind != git.Addition {
		t.Fatalf("right lines = %#v", v.Lines(right))
	}
}

func TestSplitPaneSwitchingFindsRowsAboveAndBelowEmptyTargets(t *testing.T) {
	p := git.Patch{Files: []git.File{{DisplayPath: "file", Hunks: []git.Hunk{{Header: "@@", Lines: []git.Line{
		{Kind: git.Addition, Text: "right", NewNumber: 1},
		{Kind: git.Context, Text: "both", OldNumber: 1, NewNumber: 2},
		{Kind: git.Deletion, Text: "left", OldNumber: 2},
	}}}}}}
...
func TestSplitPaneSwitchDoesNothingWhenTargetPaneIsEmpty(t *testing.T) {
	p := git.Patch{Files: []git.File{{DisplayPath: "file", Hunks: []git.Hunk{{Header: "@@", Lines: []git.Line{{Kind: git.Addition, Text: "one", NewNumber: 1}, {Kind: git.Addition, Text: "two", NewNumber: 2}}}}}}}
	v := NewSideBySideView(p, true)
...
func TestSplitVerticalMovementSkipsEmptyActivePane(t *testing.T) {
	p := git.Patch{Files: []git.File{{DisplayPath: "file", Hunks: []git.Hunk{{Header: "@@", Lines: []git.Line{{Kind: git.Context, Text: "one", OldNumber: 1, NewNumber: 1}, {Kind: git.Addition, Text: "right", NewNumber: 2}, {Kind: git.Context, Text: "two", OldNumber: 2, NewNumber: 3}, {Kind: git.Deletion, Text: "left", OldNumber: 3}, {Kind: git.Context, Text: "three", OldNumber: 4, NewNumber: 4}}}}}}}
	v := NewSideBySideView(p, true)
	first := mustFirst(t, v)
...
func TestSplitVerticalMovementAndHalfPageUseVisualRows(t *testing.T) {
	lines := []git.Line{
		{Kind: git.Deletion, Text: "d1", OldNumber: 1}, {Kind: git.Addition, Text: "a1", NewNumber: 1}, {Kind: git.Context, Text: "c1", OldNumber: 2, NewNumber: 2},
		{Kind: git.Deletion, Text: "d2", OldNumber: 3}, {Kind: git.Addition, Text: "a2", NewNumber: 3}, {Kind: git.Context, Text: "c2", OldNumber: 4, NewNumber: 4},
		{Kind: git.Deletion, Text: "d3", OldNumber: 5}, {Kind: git.Addition, Text: "a3", NewNumber: 5}, {Kind: git.Context, Text: "c3", OldNumber: 6, NewNumber: 6},
	}
	v := NewSideBySideView(git.Patch{Files: []git.File{{DisplayPath: "file", Hunks: []git.Hunk{{Header: "@@", Lines: lines}}}}}, true)
	cursor := mustFirst(t, v)
...
func TestFileHeaderSticksWithoutCoveringDiffRows(t *testing.T) {
	p := git.Patch{Files: []git.File{
		{DisplayPath: "first.go", Hunks: []git.Hunk{{Header: "@@", Lines: []git.Line{
			{Kind: git.Context, Text: "first one", OldNumber: 1, NewNumber: 1},
			{Kind: git.Context, Text: "first two", OldNumber: 2, NewNumber: 2},
			{Kind: git.Context, Text: "first three", OldNumber: 3, NewNumber: 3},
		}}}},
		{DisplayPath: "second.go", Hunks: []git.Hunk{{Header: "@@", Lines: []git.Line{
			{Kind: git.Context, Text: "second one", OldNumber: 1, NewNumber: 1},
		}}}},
	}}
...
func TestSplitTabsDoNotShiftLineNumbersOrDivider(t *testing.T) {
	p := git.Patch{Files: []git.File{{DisplayPath: "file", Hunks: []git.Hunk{{Header: "@@", Lines: []git.Line{{Kind: git.Context, Text: "\t\tlong line", OldNumber: 1, NewNumber: 1}}}}}}}
	v := NewSideBySideView(p, true)
	cursor := mustFirst(t, v)
...
func TestSyntaxHighlightingSurvivesDiffStyling(t *testing.T) {
	p := git.Patch{Files: []git.File{{DisplayPath: "main.go", NewPath: "main.go", OldSource: "package main\nold()\n", NewSource: "package main\nnew()\n", Hunks: []git.Hunk{{Header: "@@", Lines: []git.Line{{Kind: git.Deletion, Text: "old()", OldNumber: 2}, {Kind: git.Addition, Text: "new()", NewNumber: 2}}}}}}}
	v := NewUnifiedView(p, true)
	first := mustFirst(t, v)
...
func TestRenderedCodeRowsHaveExactTerminalWidth(t *testing.T) {
	for _, test := range []struct {
		constructor func(git.Patch, bool) View
		width       int
...
func TestRenderStyledRowStripsSyntaxBackgroundColors(t *testing.T) {
	...
	rendered := renderStyledRow(lineStyle(git.Addition, true), value, 80, false)
...
func longLinePatch() git.Patch {
	return git.Patch{Files: []git.File{{DisplayPath: "long", Hunks: []git.Hunk{{Header: "@@", Lines: []git.Line{{Kind: git.Context, Text: "abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ", OldNumber: 1, NewNumber: 1}}}}}}}
}
