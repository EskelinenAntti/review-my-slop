...
func (v *diffView) Render(viewport Viewport, cursor Cursor, selection *Selection) string {
	...
	if v.hasStickyHeader(viewport.Top, viewport.Height) {
		...
		lines = append(lines, v.renderFileRow(v.changes.Files[current.file].DisplayPath, viewport.Width))
	}
...
func (v *diffView) renderUnifiedRow(current row, y int, viewport Viewport, cursor Cursor, selection *Selection) string {
	...
	case lineRow:
		line := v.changes.Files[current.file].Hunks[current.hunk].Lines[current.rightLine]
		prefix := " "
		...
		if cursor.Coordinate.Y == y {
			style, strip = diffCursorStyle, true
		}
...
func (v *diffView) renderPane(current row, y int, pane Pane, width, offset int, cursor Cursor, selection *Selection) string {
	...
	line := v.changes.Files[current.file].Hunks[current.hunk].Lines[index]
	text := current.right
...
var (
	...
	diffCursorStyle  = lipgloss.NewStyle().Reverse(true)
)
