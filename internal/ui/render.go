...
func changeLineCounts(changes diff.ChangeSet) (added, removed int) {
...
func (m Model) renderComments() string {
	...
	if len(m.comments.items) == 0 {
		...
		for index := start; index < end; index++ {
			item := m.comments.items[index]
			prefix, style := "  ", contextStyle
			if index == m.comments.row {
				prefix, style = "> ", cursorStyle
			}
			location := item.Anchor.FilePath
			if item.Anchor.NewStart > 0 {
				location += fmt.Sprintf(":%d", item.Anchor.NewStart)
			} else if item.Anchor.OldStart > 0 {
