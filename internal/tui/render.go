...
func (m Model) render() string {
	...
	return m.renderScreen(header, m.renderDiffBody(), footer)
}

func (m Model) renderDiffBody() []string {
	if len(m.review.patch.Files) > 0 {
		return strings.Split(m.review.view.Render(m.review.viewport, m.review.cursor, m.review.selection), "\n")
	}
...
func (m Model) contentWidth() int { return max(minimumContentWidth, m.width) }
...
func (m Model) viewLabel() string {
	...
	if branch := m.comparisonBranch(); branch != "" {
...
func (m Model) renderCommentBody() []string {
	height := m.bodyHeight()
	...
	for index := start; index < end; index++ {
		...
		line := ansi.Truncate(fmt.Sprintf("%s%s  %s", prefix, location, commentBody), m.contentWidth(), "")
		body = append(body, style.Width(m.contentWidth()).Render(line))
	}
	return body
}

func commentLocation(comment review.Comment) string {
	location := comment.Anchor.FilePath
	if comment.Anchor.NewStart > 0 {
		return location + fmt.Sprintf(":%d", comment.Anchor.NewStart)
	}
	if comment.Anchor.OldStart > 0 {
		return location + fmt.Sprintf(":%d", comment.Anchor.OldStart)
	}
	return location
}
...
var (
	...
	commentRowStyle   = lipgloss.NewStyle()
